...
def get_random_products_for_category(session: Session, category_id: int, top_n: int = 10):
    return (
        session.query(Product)
...
def get_or_create_user(session: Session, telegram_id: str, **kwargs):
...
# Сохранить сообщение
def save_message(session: Session, user_id: int, role: str, message: str, session_id: str = ""):
...
def get_user_history(session: Session, user_id: int, limit: int = 10):
    return (
        ...
        .limit(limit)
        .all()
    )[::-1]  # вернуть в хронологическом порядке 