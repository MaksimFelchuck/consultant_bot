...
from repository import category_repo, product_repo, user_repo, message_repo, search_service
...
async def handle_message(message: Message, state: FSMContext):
    ...
    if ("категория" not in params or not params["категория"] or any(w in params["категория"] for w in IgnoreWords.WORDS)):
    ...
    if "категория" in params and not any(w in params["категория"] for w in IgnoreWords.WORDS):
    ...
    if "характеристики" in params and not any(w in params["характеристики"] for w in IgnoreWords.WORDS):
    ...
    if products:
        ...
        extra["last_products"] = [{"id": p.id, "name": p.name, "image_url": p.image_url, "desc": p.description, "price": p.price} for p in products]
        ...
        cleaned_fallback = "\n".join(
            line for line in fallback_reply.splitlines()
            if not (line.strip().lower().startswith("параметры поиска:") or 
                   line.strip().lower().startswith("извлечённые параметры:"))
        ).strip()
        ...
        if cleaned_fallback:
            ...
            await message.answer("К сожалению, не удалось найти подходящие товары. Попробуйте изменить параметры поиска.")
...
async def handle_any_photo_request(message: Message, state: FSMContext):
    ...
    if not last_products:
        await message.answer("Сначала найдите товары, а затем запросите фото конкретного товара.")
        return
    
    choice = re.sub(r"(?i)фото|картинка|photo|picture|покажи|show", "", user_message).strip()
    if not choice:
        await message.answer("Укажите номер или название товара, фото которого хотите увидеть.")
        return
...
async def handle_product_choice(message: Message, state: FSMContext):
    ...
    last_products = extra.get("last_products", [])
    if not last_products:
        await message.answer("Список товаров устарел. Пожалуйста, выполните новый поиск.")
        await state.clear()
        return
    ...
    try:
        ...
        for prod in last_products:
            if prod["name"].lower() in user_message.lower() or user_message.lower() in prod["name"].lower():
                chosen = prod
                break
    ...
    if any(word in user_message_lower for word in ["фото", "картинка", "photo", "picture", "покажи", "show"]):
        if current_product["image_url"].startswith("images/"):
            ...
            if file_path.exists():
                ...
                await message.answer(Messages.PHOTO_NOT_FOUND.format(name=current_product["name"]))
        else:
